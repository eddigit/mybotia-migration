(function() {
  ...
  // === Tab notification for background responses ===
  var _originalTitle = document.title;
  var _tabNotifTimer = null;
  function _notifyTab(agentName) {
    if (document.hasFocus()) return;
    var msg = (agentName || 'IA') + ' a répondu';
    document.title = '● ' + msg + ' — ' + _originalTitle;
    clearInterval(_tabNotifTimer);
    _tabNotifTimer = setInterval(function() {
      document.title = document.title.indexOf('●') === 0 ? _originalTitle : '● ' + msg + ' — ' + _originalTitle;
    }, 2000);
  }
  ...
  function _trapFocus(box) {
    box.addEventListener('keydown', function(e) {
      if (e.key !== 'Tab') return;
      var focusable = box.querySelectorAll('button, input, textarea, select, [tabindex]:not([tabindex="-1"])');
  ...
  var _csrfToken = '';         // CSRF token from server

  // Helper: returns headers object with CSRF token for state-changing requests
  function _csrfHeaders(extra) {
    var h = extra || {};
    if (_csrfToken) h['X-CSRF-Token'] = _csrfToken;
    return h;
  }
  window._mybotiaCsrfToken = function() { return _csrfToken; };
  window._mybotiaAuthType = function() { return _authType; };
  ...
  function destroyAuthSession() {
    fetch('/api/auth.php', { method: 'DELETE', headers: _csrfHeaders(), credentials: 'same-origin' }).catch(function() {});
  }
  ...
  function fetchSystemStatus() {
    ...
    fetch('/api/status.php', { headers: { 'X-Gateway-Token': TOKEN }, credentials: 'same-origin' })
  ...
  function renderStatusPanel(data) {
    ...
    order.forEach(function(key) {
      ...
      html += '<div class="svc-info">';
      html += '<div class="svc-label">' + escapeHTML(svc.label) + '</div>';
      html += '<div class="svc-detail">' + escapeHTML(svc.detail) + '</div>';
      html += '</div></div>';
    });
  ...
  // Markdown LRU cache: avoid re-parsing finalized messages
  var _mdCacheMap = new Map();
  var _mdCacheMax = 20;
  function _mdCacheGet(key) {
    if (!_mdCacheMap.has(key)) return null;
    var val = _mdCacheMap.get(key);
    _mdCacheMap.delete(key);
    _mdCacheMap.set(key, val); // Move to end (most recent)
    return val;
  }
  function _mdCacheSet(key, val) {
    if (_mdCacheMap.size >= _mdCacheMax) {
      _mdCacheMap.delete(_mdCacheMap.keys().next().value); // Evict oldest
    }
    _mdCacheMap.set(key, val);
  }

  function parseMarkdown(text) {
    if (!text) return '';
    var cached = _mdCacheGet(text);
    ...
    html = html.replace(/```(\w*)\n([\s\S]*?)```/g, function(m, lang, code) {
      ...
      codeBlocks.push('<div class="code-wrapper' + wrapClass + '">' + langLabel + '<pre><code' + langClass + '>' + code.trim() + '</code></pre></div>');
  ...
  function addMessage(role, text, timestamp, images) {
    ...
    if(role==='assistant'){addCopyButtons(bubble);if(window.Prism)Prism.highlightAllUnder(bubble);if(!_batchFragment)addLinkPreviews(bubble)}
  }
  ...
  function _showMessageSkeleton() {
    while ($messages.firstChild) $messages.removeChild($messages.firstChild);
    var frag = document.createDocumentFragment();
    for (var i = 0; i < 4; i++) {
      var sk = document.createElement('div');
      sk.className = 'message-skeleton' + (i % 2 === 0 ? ' sk-assistant' : ' sk-user');
      sk.innerHTML = '<div class="sk-avatar"></div><div class="sk-lines">' +
        '<div class="sk-line" style="width:' + (60 + Math.random() * 30) + '%"></div>' +
        '<div class="sk-line" style="width:' + (40 + Math.random() * 40) + '%"></div>' +
        (i % 2 === 0 ? '<div class="sk-line" style="width:' + (30 + Math.random() * 30) + '%"></div>' : '') +
        '</div>';
      frag.appendChild(sk);
    }
  ...
  function connectWS() {
    ...
    ws.addEventListener('close', function(evt) {
      ...
      ws = null;
      setStatus('disconnected');
      // Reset generation state to unblock UI
      if (_isGenerating) {
        setGenerating(false);
        hideTyping();
        hideStreaming();
  ...
  function sendConnect() {
    ...
    request('connect', connectParams).then(function(payload) {
      ...
      loadPrefsFromServer().then(function(prefs) {
        loadSessions();
      }).catch(function() {
        loadSessions();
      });
  ...
  function handleMessage(data) {
    ...
    if (data.type === 'event') {
      ...
      // Agent lifecycle events — IGNORED
      // These events have no sessionKey filter, so WhatsApp/Telegram activity
      // from other sessions leaks into the webchat UI. The chat delta/final
      // events (which DO have session filtering) are sufficient to drive the UI.
      return;
    }
  ...
  function showToolIndicator(hint) {
    ...
    // Single compact indicator — updates in-place, never stacks
    if (!_toolStepsEl) {
      _toolStepsEl = document.createElement('div');
      _toolStepsEl.className = 'tool-steps';
      _toolStepsEl.innerHTML = '<div class="tool-step"><span class="tool-step-dot"></span><span class="tool-step-text"></span><span class="tool-step-count"></span></div>';
      _toolStepCount = 0;
    }

    var stepText = _toolStepsEl.querySelector('.tool-step-text');
    var stepCount = _toolStepsEl.querySelector('.tool-step-count');
    var currentHint = stepText ? stepText.textContent : '';

    if (currentHint === hint) {
      // Same tool — just pulse
      return;
    }

    // Update the single indicator text
    if (stepText) stepText.textContent = hint;
    _toolStepCount++;
    if (stepCount && _toolStepCount > 1) {
      stepCount.textContent = _toolStepCount + ' actions';
    }

    // Insert into DOM if not yet
  ...
  function hideToolIndicator() {
    ...
    // Fade out and remove the single compact indicator
    if (_toolStepsEl) {
      _toolStepsEl.classList.add('fade-out');
      var elRef = _toolStepsEl;
      setTimeout(function() {
        if (elRef && elRef.parentNode) elRef.parentNode.removeChild(elRef);
      }, 300);
      _toolStepsEl = null;
      _toolStepCount = 0;
    }
  ...
  function handleChatEvent(payload) {
    ...
    if (!isCurrentSession) {
      ...
      if (payload.state === 'final') {
        // Invalidate history cache for this session (new message arrived)
        if (eventKey) _invalidateHistoryCache(eventKey);
    ...
    if (payload.state === 'delta') {
      ...
      // Smooth transition: promote streaming content to final message (no flash)
      if (finalText && $streaming.parentNode && $streaming.classList.contains('visible')) {
        // Parse final text into the streaming bubble directly
        clearTimeout(_streamActivityTimer);
        if (_streamParseTimer) { clearTimeout(_streamParseTimer); _streamParseTimer = null; }
        $streamingText.innerHTML = parseMarkdown(finalText);
        _streamRawText = '';

        // Now create the final message element and swap
        var _finalTs = payload.message ? payload.message.timestamp : Date.now();
        $streaming.classList.remove('visible');
        $streaming.classList.remove('working');
        if ($streaming.parentNode) $streaming.parentNode.removeChild($streaming);
        $streamingText.textContent = '';
        addMessage('assistant', finalText, _finalTs, finalImages);
      } else {
        hideStreaming();
        if (finalText) {
      ...
      setGenerating(false);
      chatRunId = null;
      if (finalText) trackUsage('received', finalText.length);
      streamText = '';
      // Invalidate history cache (new message in current session)
      _invalidateHistoryCache(SESSION_KEY);
      // Smart sidebar update: just update the active item's timestamp + preview
      updateSidebarActiveItem(finalText);
      // Notify tab if in background
      var _agName = document.getElementById('header-name');
      _notifyTab(_agName ? _agName.textContent : null);
  ...
  var _historyCache = new Map(); // session key -> { messages, ts }
  var _historyCacheTTL = 5 * 60 * 1000; // 5 minutes

  function _invalidateHistoryCache(sessionKey) {
    // Invalidate by any matching key (fuzzy like session matching)
    _historyCache.forEach(function(val, key) {
      if (key === sessionKey || key.indexOf(sessionKey) !== -1 || sessionKey.indexOf(key) !== -1) {
  ...
  function loadHistory() {
    ...
    // Check cache first
    var cached = _historyCache.get(SESSION_KEY);
    if (cached && (Date.now() - cached.ts < _historyCacheTTL)) {
      _renderHistory(cached.messages);
      return;
    }
    ...
    request('chat.history', { sessionKey: SESSION_KEY, limit: 200 }).then(function(result) {
      ...
      // Cache the result
      _historyCache.set(SESSION_KEY, { messages: messages, ts: Date.now() });
  ...
  function sendMessage() {
    if (_isGenerating) return; // Prevent double-send
  ...
  function loadPrefsFromServer() {
    ...
    return fetch(url).then(function(res) {
      ...
      // Sync session renames: server wins
      if (prefs.session_renames) {
        try {
          var serverRenames = (typeof prefs.session_renames === 'string') ? JSON.parse(prefs.session_renames) : prefs.session_renames;
          if (serverRenames && typeof serverRenames === 'object') {
            localStorage.setItem('mybotia-session-renames', JSON.stringify(serverRenames));
          }
        } catch(e) {}
      }

      // Sync pinned: server wins
      if (prefs.pinned) {
        try {
          var serverPinned = (typeof prefs.pinned === 'string') ? JSON.parse(prefs.pinned) : prefs.pinned;
          if (Array.isArray(serverPinned)) {
            localStorage.setItem('mybotia-pinned', JSON.stringify(serverPinned));
          }
        } catch(e) {}
      }
      ...
      if (!prefs.session_renames) {
        var localRenames = localStorage.getItem('mybotia-session-renames');
        if (localRenames) {
          syncPrefToServer('session_renames', localRenames);
          console.log('[prefs] Migrated local session_renames to server');
        }
      }
      if (!prefs.pinned) {
        var localPinned = localStorage.getItem('mybotia-pinned');
        if (localPinned) {
          syncPrefToServer('pinned', localPinned);
          console.log('[prefs] Migrated local pinned to server');
        }
      }
  ...
  function saveSessionRenames(map) {
    ...
    syncPrefToServer('session_renames', JSON.stringify(map));
  }
  ...
  function switchSession(newKey) {
    ...
    // Show skeleton loader while history loads
    _showMessageSkeleton();

    // Notify tools-panel of session change
    try { window.dispatchEvent(new CustomEvent('mybotia-session-change', { detail: { sessionKey: newKey } })); } catch(e) {}
        ...
        .then(function(data) {
          ...
          if ($histBars) {
            ...
            for (var d = 6; d >= 0; d--) {
              ...
              var val = (history && history[key]) ? history[key] : 0;
  ...
  function renderDynamicCards(cards) {
    ...
    cards.forEach(function(card) {
      ...
      el.innerHTML = '<div class="dashboard-card-icon">' + (_cardIconMap[card.icon] || _cardIconMap.doc) + '</div>' +
        '<div class="dashboard-card-text"><div class="dashboard-card-title">' + escapeHTML(card.title) + '</div>' +
        '<div class="dashboard-card-desc">' + escapeHTML(card.desc) + '</div></div>';
    ...
    if (data.client) {
      ...
      if ($avatar) {
        if (data.client.photo_url) {
          $avatar.innerHTML = '<img src="' + escapeHTML(data.client.photo_url) + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="">';
        } else {
  ...
  function renderLinkPreview(bubble, data) {
    ...
    if (data.site_name) html += '<div class="link-preview-site">' + escapeHTML(data.site_name) + '</div>';
  ...
  // === Voice Settings in Parametres ===
  var $voiceSection = document.getElementById('settings-voice-section');
  var $voiceMic = document.getElementById('settings-voice-mic');
  var $voiceSpeed = document.getElementById('settings-voice-speed');
  var $voiceSpeedVal = document.getElementById('settings-voice-speed-val');
  var $voiceMode = document.getElementById('settings-voice-mode');
  var $voiceEmail = document.getElementById('settings-voice-email');

  function initVoiceSettings() {
    if (!$voiceSection) return;
    // Show section only if voice-config endpoint responds ok
    fetch('/api/voice-config.php', { credentials: 'same-origin' })
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data && data.ok) $voiceSection.style.display = '';
      })
      .catch(function() {});
  }

  function populateVoiceMics() {
    if (!$voiceMic || !navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) return;
    navigator.mediaDevices.enumerateDevices().then(function(devices) {
      var saved = localStorage.getItem('voice-app:mic-id') || '';
      // Clear existing options except default
      $voiceMic.innerHTML = '<option value="">Micro par defaut</option>';
      devices.forEach(function(d) {
        if (d.kind !== 'audioinput') return;
        var opt = document.createElement('option');
        opt.value = d.deviceId;
        opt.textContent = d.label || ('Micro ' + d.deviceId.substring(0, 8));
  ...
  function loadVoiceSettingsValues() {
    if ($voiceSpeed) {
      var speed = parseInt(localStorage.getItem('voice-app:speed') || '-5', 10);
      $voiceSpeed.value = speed;
      if ($voiceSpeedVal) $voiceSpeedVal.textContent = speed;
    }
    if ($voiceMode) {
      $voiceMode.value = localStorage.getItem('voice-app:default-mode') || 'free';
    }
    if ($voiceEmail) {
      $voiceEmail.value = localStorage.getItem('voice-app:report-email') || '';
    }
    populateVoiceMics();
  }

  if ($voiceMic) {
    $voiceMic.addEventListener('change', function() {
      localStorage.setItem('voice-app:mic-id', $voiceMic.value);
      if (typeof VoiceClient !== 'undefined' && VoiceClient.updateMic) VoiceClient.updateMic($voiceMic.value);
    });
  }
  if ($voiceSpeed) {
    $voiceSpeed.addEventListener('input', function() {
      if ($voiceSpeedVal) $voiceSpeedVal.textContent = $voiceSpeed.value;
      localStorage.setItem('voice-app:speed', $voiceSpeed.value);
      if (typeof VoiceClient !== 'undefined' && VoiceClient.updateRate) VoiceClient.updateRate(parseInt($voiceSpeed.value, 10));
    });
  }
  if ($voiceMode) {
    $voiceMode.addEventListener('change', function() {
      localStorage.setItem('voice-app:default-mode', $voiceMode.value);
    });
  }
  if ($voiceEmail) {
    $voiceEmail.addEventListener('change', function() {
      localStorage.setItem('voice-app:report-email', $voiceEmail.value.trim());
    });
  }

  // Hook into openSettingsPanel
  var _origOpenSettings = openSettingsPanel;
  openSettingsPanel = function() {
    _origOpenSettings();
    loadVoiceSettingsValues();
  };
  ...
  var _cmdActions = [
    { id: 'new', icon: '➕', label: 'Nouvelle conversation', shortcut: 'Ctrl+N', action: function() { document.getElementById('btn-new-chat').click(); } },
    ...
    { id: 'tools', icon: '🛠', label: 'Ouvrir/fermer le panneau outils', shortcut: 'Ctrl+.', action: function() {
  ...
  document.addEventListener('keydown', function(e) {
    ...
    // Skip if inside an input/textarea (except for palette and escape)
    ...
    // Escape — Close open panel/modal/palette
    if (e.key === 'Escape') {
      if (_cmdPaletteOpen) { closeCommandPalette(); return; }
      // Close right sidebar tools panel
      var rp = document.getElementById('right-sidebar');
      if (rp && rp.classList.contains('open')) {
        var tb = document.getElementById('btn-tools-toggle');
        if (tb) { tb.click(); return; }
      }
      // Close mobile left sidebar
      if ($sidebar && $sidebar.classList.contains('open')) {
        $sidebar.classList.remove('open');
        if ($sidebarOverlay) $sidebarOverlay.classList.remove('visible');
        return;
      }
      // Close settings/profile panels if open
    ...
    // Shortcuts below should not fire when typing in inputs
    if (inInput) return;

    // Ctrl+N — New conversation
    if (mod && e.key === 'n') {
      e.preventDefault();
      var nc = document.getElementById('btn-new-chat');
      if (nc) nc.click();
      return;
    }
    ...
    // Ctrl+. — Toggle right sidebar (tools)
    if (mod && e.key === '.') {
      e.preventDefault();
      var toolsBtn = document.getElementById('btn-tools-toggle');
      if (toolsBtn) toolsBtn.click();
    ...
    // / — Focus chat input (quick shortcut like Slack)
  ...
  var _voiceBubbleIds = []; // track injected voice bubbles for potential removal

  // Real-time transcript injection into main chat
  window._onVoiceTranscriptEntry = function(entry) {
    var role = entry.role === 'user' ? 'user' : 'assistant';
    var text = entry.text || '';
    if (!text) return;

    // Build voice message with waveform indicator
    var voiceIndicator = '<span class="voice-indicator" style="opacity:0.5;font-size:11px;margin-right:4px;"><i class="ph-thin ph-waveform"></i></span>';

    var msgDiv = document.createElement('div');
    msgDiv.className = 'msg ' + role;
    msgDiv.setAttribute('data-voice', 'true');
    var bubbleId = 'voice-msg-' + Date.now() + '-' + Math.random().toString(36).substr(2, 5);
    msgDiv.id = bubbleId;
    _voiceBubbleIds.push(bubbleId);
    ...
    if (role === 'assistant') {
      var textDiv = document.createElement('div');
      textDiv.innerHTML = voiceIndicator + parseMarkdown(text);
      bubble.appendChild(textDiv);
    } else {
      var textDiv = document.createElement('div');
      textDiv.className = 'msg-text';
      textDiv.innerHTML = voiceIndicator + escapeHTML(text);
      bubble.appendChild(textDiv);
    }
  ...
  if (btnVoice) {
    btnVoice.addEventListener('click', async function() {
      if (typeof VoiceClient === 'undefined') return;

      if (VoiceClient.isOpen()) {
        VoiceClient.close();
        return;
      }

      // Fetch voice config if not cached
      if (!_voiceConfig) {
        try {
          var resp = await fetch('/api/voice-config.php', { credentials: 'same-origin' });
          if (!resp.ok) {
            console.warn('[Voice] Config fetch failed:', resp.status);
            return;
          }
          _voiceConfig = await resp.json();
        } catch (e) {
      ...
      // Init VoiceClient if first time
      if (!_voiceReady && _voiceConfig && _voiceConfig.ok) {
        var vc = _voiceConfig;
        VoiceClient.init({
          agentConfig: {
            name: vc.agent.name,
            voice: vc.agent.voice,
            voiceRate: vc.agent.voiceRate,
            voicePitch: vc.agent.voicePitch,
            wakeWord: vc.agent.wakeWord,
            systemPrompt: vc.agent.systemPrompt,
            gatewayToken: vc.gateway.token,
            userName: vc.user.name,
            avatar: vc.agent.avatar,
            role: vc.agent.role
          },
          sessionKey: SESSION_KEY
        });
      ...
      // Callback when voice overlay closes
      window._onVoiceClosed = function(result) {
        if (!result) {
          // Discard: remove all voice bubbles from chat
          _voiceBubbleIds.forEach(function(id) {
            var el = document.getElementById(id);
            if (el) el.remove();
          });
          _voiceBubbleIds = [];
        ...
        if (result.action === 'keep') {
          // Transcripts already in chat — nothing to do
          _voiceBubbleIds = [];
        } else if (result.action === 'summary') {
          // Remove individual bubbles, request summary instead
          _voiceBubbleIds.forEach(function(id) {
            var el = document.getElementById(id);
            if (el) el.remove();
          });
          ...
          if (result.transcript && result.transcript.length > 0) {
            var summaryText = result.transcript.map(function(e) {
              return (e.role === 'user' ? 'Utilisateur' : 'Agent') + ' : ' + e.text;
            }).join('\n');
            var summaryPrompt = '[Resume de conversation vocale]\nResume les points cles de cette conversation vocale en quelques lignes :\n\n' + summaryText;
