...
(function() {
  ...
  // CSRF helper — reads token from main app
  function _tpCsrfHeaders(extra) {
    var h = extra || {};
    var token = (typeof window._mybotiaCsrfToken === 'function') ? window._mybotiaCsrfToken() : '';
  ...
  function initToggle() {
    ...
    if (btn) {
      btn.addEventListener('click', function() {
        ...
        var vp = document.getElementById('voice-panel');
        if (vp && vp.classList.contains('open')) {
          if (typeof VoiceClient !== 'undefined') VoiceClient.close();
        }
  ...
  // --- Notes (server-side sync) ---
  var notesSaveTimeout = null;
  var _notesCache = {}; // in-memory cache per session

  function _getToken() {
    return document.cookie.split(';').reduce(function(t, c) {
      var p = c.trim().split('='); return p[0] === 'mybotia-token' ? p[1] : t;
    }, '') || (window._mybotiaToken || '');
  }

  function _isEmailAuth() {
    return document.body.dataset.authType === 'email' || (window._mybotiaAuthType && window._mybotiaAuthType() === 'email');
  }

  function _loadNotesFromServer(callback) {
    if (_isEmailAuth()) {
      // Session-based notes API for email-auth clients
      fetch('/api/notes.php', { credentials: 'same-origin' })
        .then(function(r) { return r.json(); })
        .then(function(data) {
          if (data.ok && data.notes) _notesCache = data.notes;
          callback(_notesCache);
        }).catch(function() { callback(null); });
      return;
    }
    // Legacy token-based preferences API
    var token = _getToken();
    if (!token) { callback(null); return; }
    fetch('/api/preferences.php', {
      headers: { 'X-Gateway-Token': token }
    }).then(function(r) { return r.json(); }).then(function(data) {
      if (data.ok && data.preferences && data.preferences.notes) {
        try {
          _notesCache = (typeof data.preferences.notes === 'string') ? JSON.parse(data.preferences.notes) : data.preferences.notes;
        } catch(e) { _notesCache = {}; }
      }
      callback(_notesCache);
    }).catch(function() { callback(null); });
  ...
  function _saveNotesToServer(sessionKey) {
    if (_isEmailAuth()) {
      var sk = sessionKey || localStorage.getItem('mybotia-session') || 'main';
      fetch('/api/notes.php', {
        method: 'POST',
        headers: _tpCsrfHeaders({ 'Content-Type': 'application/json' }),
        credentials: 'same-origin',
        body: JSON.stringify({ session_key: sk, content: _notesCache[sk] || '' })
      }).catch(function() {});
      return;
    }
    var token = _getToken();
    ...
    fetch('/api/preferences.php', {
      method: 'POST',
      headers: _tpCsrfHeaders({ 'Content-Type': 'application/json', 'X-Gateway-Token': token }),
      body: JSON.stringify({ notes: JSON.stringify(_notesCache) })
    }).catch(function() {});
  }
  ...
  function initNotes() {
    ...
    // Load from server
    _loadNotesFromServer(function(cache) {
      if (cache && cache[sessionKey]) {
        area.value = cache[sessionKey];
      } else {
        // Fallback: migrate from localStorage if present
        var localSaved = localStorage.getItem(NOTES_KEY + sessionKey);
        if (localSaved) {
          area.value = localSaved;
          _notesCache[sessionKey] = localSaved;
          _saveNotesToServer();
          localStorage.removeItem(NOTES_KEY + sessionKey);
        }
      }
    });
    ...
    area.addEventListener('input', function() {
      ...
      notesSaveTimeout = setTimeout(function() {
        ...
        _notesCache[key] = area.value;
        _saveNotesToServer();
        status.textContent = 'Sauvegardé — ' + new Date().toLocaleTimeString('fr-FR');
      }, 800);
    });

    // Listen for session changes (custom event from app.js)
    window.addEventListener('mybotia-session-change', function(e) {
      var newKey = (e.detail && e.detail.sessionKey) || localStorage.getItem('mybotia-session') || 'main';
      area.value = _notesCache[newKey] || '';
      status.textContent = '';
    });
  }
    ...
    fetch(CRM_API + '?action=client&id=' + clientId).then(function(r) { return r.json(); }).then(function(data) {
      ...
      if (delBtn) {
        delBtn.addEventListener('click', function() {
          ...
          fetch(CRM_API + '?action=client&id=' + delBtn.dataset.clientId, { method: 'DELETE', headers: _tpCsrfHeaders() })
            .then(function(r) { return r.json(); })
  ...
  function deleteTask(taskId, itemEl) {
    ...
    fetch(TASKS_API_URL + '?id=' + taskId, { method: 'DELETE', headers: _tpCsrfHeaders() })
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data.success) {
          if (itemEl) {
