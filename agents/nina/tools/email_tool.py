...
def cmd_send(config, to, subject, body, reply_to_id=None, reply_msg=None, cc=None):
    """Envoie un mail."""
    msg = MIMEMultipart()
...
def main():
    ...
    try:
        if cmd == "check":
            ...
            if len(sys.argv) < 5:
                print("Usage: email_tool.py send <to> <subject> <body> [--cc <email>]")
                sys.exit(1)
            cc = None
            if "--cc" in sys.argv:
                cc_index = sys.argv.index("--cc")
                if cc_index + 1 < len(sys.argv):
                    cc = sys.argv[cc_index + 1]
            cmd_send(config, sys.argv[2], sys.argv[3], sys.argv[4], cc=cc)
        elif cmd == "draft":
            if len(sys.argv) < 5:
