...
- French (Lucy): `fr-FR-VivienneMultilingualNeural` (voix féminine)
...
```bash
scripts/gen_voice.sh "Bonjour, comment puis-je vous aider ?" /tmp/voice.ogg
scripts/gen_voice.sh "Hello" /tmp/voice.ogg en-US-JennyNeural
```

Arguments: `<text> <output.ogg> [voice]`
- If voice is omitted, defaults to `fr-FR-VivienneMultilingualNeural`.
